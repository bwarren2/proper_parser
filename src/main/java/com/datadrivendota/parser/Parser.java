    ...
    public void onTickStart(Context ctx, boolean synthetic) {
        ...
        if(tick_time >= next_time && playerresource!=null){
            ...
            for (int i =0; i<10; i++) {
                ...
                state.hero_id = getEntityProperty(playerresource, "m_vecPlayerTeamData.%i.m_nSelectedHeroID", i);
                state.kills = getEntityProperty(playerresource, "m_vecPlayerTeamData.%i.m_iKills", i);
                state.assists = getEntityProperty(playerresource, "m_vecPlayerTeamData.%i.m_iAssists", i);
                state.deaths = getEntityProperty(playerresource,"m_vecPlayerTeamData.%i.m_iDeaths", i);
                ...
                state.total_earned_gold = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iTotalEarnedGold", lookup_i);
                state.reliable_gold = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iReliableGold", lookup_i);
                state.unreliable_gold = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iUnreliableGold", lookup_i);
                state.xp = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iTotalEarnedXP", lookup_i);
                state.shared_gold = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iSharedGold", lookup_i);
                state.hero_kill_gold = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iHeroKillGold", lookup_i);
                state.creep_kill_gold = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iCreepKillGold", lookup_i);
                state.income_gold = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iIncomeGold", lookup_i);
                state.denies = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iDenyCount", lookup_i);
                state.last_hits = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iLastHitCount", lookup_i);
                state.nearby_creep_deaths = getEntityProperty(
                        side_resource,
                        "m_vecDataTeam.%i.m_iNearbyCreepDeathCount",
                        lookup_i
                );
                state.misses = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iMissCount", lookup_i);
                state.healing = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_fHealing", lookup_i);
                state.tower_kills = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iTowerKills", lookup_i);
                state.roshan_kills = getEntityProperty(side_resource, "m_vecDataTeam.%i.m_iRoshanKills", lookup_i);

                int hero_handle = getEntityProperty(playerresource, "m_vecPlayerTeamData.%i.m_hSelectedHero", i);
                ...
                if (hero != null) {
                    state.x = getEntityProperty(hero, "CBodyComponent.m_cellX", null);
                    state.y = getEntityProperty(hero, "CBodyComponent.m_cellY", null);

                    state.health = getEntityProperty(hero, "m_iHealth", null);
                    state.max_health = getEntityProperty(hero, "m_iMaxHealth", null);
                    state.mana = getEntityProperty(hero, "m_flMana", null);
                    state.max_mana = getEntityProperty(hero, "m_flMaxMana", null);
                    state.max_mana = getEntityProperty(hero, "m_flMaxMana", null);

                    state.damage_max = getEntityProperty(hero, "m_iDamageMax", null);
                    state.damage_min = getEntityProperty(hero, "m_iDamageMin", null);

                    state.damage_bonus = getEntityProperty(hero, "m_iDamageBonus", null);
                    state.strength = getEntityProperty(hero, "m_flStrength", null);
                    state.agility = getEntityProperty(hero, "m_flAgility", null);
                    state.intelligence = getEntityProperty(hero, "m_flIntellect", null);
                    state.strength_total = getEntityProperty(hero, "m_flStrengthTotal", null);
                    state.agility_total = getEntityProperty(hero, "m_flAgilityTotal", null);
                    state.intelligence_total = getEntityProperty(hero, "m_flIntellectTotal", null);
                    state.respawn_time = getEntityProperty(hero, "m_flRespawnTime", null);
                    state.recent_damage = getEntityProperty(hero, "m_iRecentDamage", null);
                    state.armor = getEntityProperty(hero, "m_flPhysicalArmorValue", null);
                    state.magic_resist_pct = getEntityProperty(hero, "m_flMagicalResistanceValue", null);
                    state.lifestate = getEntityProperty(hero, "m_lifeState", null);
...
//                data.stateadd(state);
    ...
    public void onStringTableEntry(Context ctx, StringTable stringTable, int rowIndex, String key, ByteString bs) {
        ...
        ste.idx = rowIndex;
        ste.value = key;
        this.filebox.addStringTableEntry(ste);
    }
